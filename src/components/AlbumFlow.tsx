...
export const AlbumFlow = ({ id, onUnsavedChangesChange }: AlbumFlowProps) => {
  ...
  const updatePageLayout = (pageIndex: number, layoutId: string) => {
    ...
    setPages((prev) => {
      const newPages = [...prev];
      const currentImages = newPages[pageIndex].images.filter(
        (img) => img !== null,
      );
      newPages[pageIndex] = {
        ...newPages[pageIndex],
        layoutId,
        images: Array(layout.imageCount)
          .fill(null)
          .map((_, i) => currentImages[i] || null),
      };
      return newPages;
    });
  };
  ...
  const handleDrop = (e: React.DragEvent, targetSlotIndex: number) => {
    ...
    setPages((prev) => {
      ...
      newPages[currentPageIndex].images[targetSlotIndex] =
        newPages[currentPageIndex].images[sourceSlotIndex];
  ...
  const createPageCanvas = async (
    page: AlbumPage,
    canvasWidth: number = 2400,
    canvasHeight: number = 2400,
  ): Promise<Blob | null> => {
    try {
  ...
  const handleSubmitOrder = async (contactData: ContactFormData) => {
    try {
      ...
      const validPageBlobs = processedPages.filter(
        (blob) => blob !== null,
      ) as Blob[];
      ...
      formData.append(
        "documents[0][no_of_pages]",
        validPageBlobs.length.toString(),
      );
  ...
  const currentLayout = ALBUM_LAYOUTS.find(
          ...
          <div className="p-4 bg-muted rounded-lg mb-4">
            <div className="flex justify-between items-center">
              ...
              <span className="text-lg font-bold">
                {BASE_PRICE + pageCount * PRICE_PER_PAGE} tk
              </span>
            </div>
          </div>
                      ...
                      {imageData ? (
                        ...
                        >
                          <ImageSlotEditor
                            ...
                            onRemove={() =>
                              removeImageFromSlot(currentPageIndex, slotIndex)
                            }
                            zoom={imageData.zoom}
